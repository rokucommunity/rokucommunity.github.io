...
For example, consider the `bs1013` warning which says `"This file is not referenced by any other file in the project"`.If you wanted to turn that into an error, update the bsconfig as follows:
...
This PR aligns on [micromatch](https://www.npmjs.com/package/micromatch) for path filtering logic, meaning that now all glob logic within roku-deploy is handled using the same library.(see [#109](https://github.com/RokuCommunity/roku-deploy/pull/109))
